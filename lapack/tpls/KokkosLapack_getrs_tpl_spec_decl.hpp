...
void lapackGetrsWrapper(const char trans[], const AView& A, const PivotView& Ipiv,
                       const BView& B, const InfoView& Info) {
  ...
  if constexpr (KokkosKernels::ArithTraits<Scalar>::is_complex) {
    ...
    HostLapack<HostScalar>::getrs(trans[0], n, nrhs, reinterpret_cast<const HostScalar*>(A.data()), lda,
                                 Ipiv.data(), reinterpret_cast<HostScalar*>(B.data()), ldb, Info.data());
  } else {
...
#define KOKKOSLAPACK_GETRS_LAPACK(SCALAR, LAYOUT, EXECSPACE, MEM_SPACE)                                                  \
  ...
  struct GETRS<                                                                                                          \
    ...
    using AViewType =                                                                                                    \
        Kokkos::View<const SCALAR**, LAYOUT, Kokkos::Device<EXECSPACE, MEM_SPACE>, Kokkos::MemoryTraits<Kokkos::Unmanaged>>; \
    using BViewType = Kokkos::View<SCALAR**, LAYOUT, Kokkos::Device<EXECSPACE, MEM_SPACE>, Kokkos::MemoryTraits<Kokkos::Unmanaged>>; \
    using IpivView =                                                                                                     \
        Kokkos::View<const int*, LAYOUT, Kokkos::Device<EXECSPACE, MEM_SPACE>, Kokkos::MemoryTraits<Kokkos::Unmanaged>>; \
    using InfoViewType =                                                                                                 \
