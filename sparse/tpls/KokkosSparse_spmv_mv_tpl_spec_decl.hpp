...
template <typename Handle, typename MatrixType, typename XViewType, typename YViewType>
inline void spmv_mv_mkl(Handle* handle, sparse_operation_t op, typename YViewType::const_value_type  alpha,
			const typename YViewType::const_value_type beta, const MatrixType &A, const XViewType &x,
			const YViewType &y) {
  using Scalar    = std::remove_cv_t<typename MatrixType::value_type>;
  using MKLScalar = typename KokkosToMKLScalar<typename YViewType::non_const_value_type>::type;
  using ExecSpace = typename Handle::ExecutionSpaceType;
  using Subhandle = MKL_SpMV_Data<ExecSpace>;
  Subhandle* subhandle;
  const MKLScalar* x_mkl = reinterpret_cast<const MKLScalar*>(x.data());
  MKLScalar* y_mkl       = reinterpret_cast<MKLScalar*>(y.data());
  if (handle->tpl_rank1) {
    ...
    MKLScalar* Avalues_mkl = reinterpret_cast<MKLScalar*>(const_cast<Scalar*>(A.values.data()));
    if constexpr (std::is_same_v<Scalar, float>) {
      KOKKOSKERNELS_MKL_SAFE_CALL(
          mkl_sparse_s_create_csr(&subhandle->mat, SPARSE_INDEX_BASE_ZERO, m, n, const_cast<MKL_INT*>(A.graph.row_map.data()),
                                  const_cast<MKL_INT*>(A.graph.row_map.data() + 1), const_cast<MKL_INT*>(A.graph.entries.data()),
				  Avalues_mkl));
    } else if constexpr (std::is_same_v<Scalar, double>) {
      KOKKOSKERNELS_MKL_SAFE_CALL(
          mkl_sparse_d_create_csr(&subhandle->mat, SPARSE_INDEX_BASE_ZERO, m, n, const_cast<MKL_INT*>(A.graph.row_map.data()),
                                  const_cast<MKL_INT*>(A.graph.row_map.data() + 1), const_cast<MKL_INT*>(A.graph.entries.data()),
				  Avalues_mkl));
    } else if constexpr (std::is_same_v<Scalar, Kokkos::complex<float>>) {
      KOKKOSKERNELS_MKL_SAFE_CALL(
          mkl_sparse_c_create_csr(&subhandle->mat, SPARSE_INDEX_BASE_ZERO, m, n, const_cast<MKL_INT*>(A.graph.row_map.data()),
                                  const_cast<MKL_INT*>(A.graph.row_map.data() + 1), const_cast<MKL_INT*>(A.graph.entries.data()),
				  Avalues_mkl));
    } else if constexpr (std::is_same_v<Scalar, Kokkos::complex<double>>) {
      KOKKOSKERNELS_MKL_SAFE_CALL(
          mkl_sparse_z_create_csr(&subhandle->mat, SPARSE_INDEX_BASE_ZERO, m, n, const_cast<MKL_INT*>(A.graph.row_map.data()),
                                  const_cast<MKL_INT*>(A.graph.row_map.data() + 1), const_cast<MKL_INT*>(A.graph.entries.data()),
				  Avalues_mkl));
    }
  }
  ...
  if constexpr (std::is_same_v<Scalar, float>) {
    KOKKOSKERNELS_MKL_SAFE_CALL(
        mkl_sparse_s_mm(op, alpha_mkl, subhandle->mat, subhandle->descr, SPARSE_LAYOUT_COLUMN_MAJOR, x_mkl,
			static_cast<MKL_INT>(y.extent(1)), static_cast<MKL_INT>(x.stride(1)), beta_mkl, y_mkl,
			static_cast<MKL_INT>(y.stride(1))));
  } else if constexpr (std::is_same_v<Scalar, double>) {
    KOKKOSKERNELS_MKL_SAFE_CALL(
        mkl_sparse_d_mm(op, alpha_mkl, subhandle->mat, subhandle->descr, SPARSE_LAYOUT_COLUMN_MAJOR, x_mkl,
			static_cast<MKL_INT>(y.extent(1)), static_cast<MKL_INT>(x.stride(1)), beta_mkl, y_mkl,
			static_cast<MKL_INT>(y.stride(1))));
  } else if constexpr (std::is_same_v<Scalar, Kokkos::complex<float>>) {
    KOKKOSKERNELS_MKL_SAFE_CALL(
        mkl_sparse_c_mm(op, alpha_mkl, subhandle->mat, subhandle->descr, SPARSE_LAYOUT_COLUMN_MAJOR, x_mkl,
			static_cast<MKL_INT>(y.extent(1)), static_cast<MKL_INT>(x.stride(1)), beta_mkl, y_mkl,
			static_cast<MKL_INT>(y.stride(1))));
  } else if constexpr (std::is_same_v<Scalar, Kokkos::complex<double>>) {
    KOKKOSKERNELS_MKL_SAFE_CALL(
        mkl_sparse_z_mm(op, alpha_mkl, subhandle->mat, subhandle->descr, SPARSE_LAYOUT_COLUMN_MAJOR, x_mkl,
			static_cast<MKL_INT>(y.extent(1)), static_cast<MKL_INT>(x.stride(1)), beta_mkl, y_mkl,
			static_cast<MKL_INT>(y.stride(1))));
  }
}
...
#define KOKKOSSPARSE_SPMV_MV_MKL(SCALAR, EXECSPACE)                                                                     \
  template <>                                                                                                           \
  struct SPMV_MV<EXECSPACE, KokkosSparse::Impl::SPMVHandleImpl<EXECSPACE, Kokkos::HostSpace, SCALAR, MKL_INT, MKL_INT>, \
    ...
    using coefficient_type = typename YVector::non_const_value_type;                                                    \
                                                                                                                        \
    static void spmv_mv(const EXECSPACE&, Handle* handle, const char mode[], const coefficient_type& alpha,             \
                     const AMatrix& A, const XVector& x, const coefficient_type& beta, const YVector& y) {              \
      std::string label = "KokkosSparse::spmv_mv[TPL_MKL," + KokkosKernels::ArithTraits<SCALAR>::name() + "]";          \
      Kokkos::Profiling::pushRegion(label);                                                                             \
      spmv_mv_mkl(handle, mode_kk_to_mkl(mode[0]), alpha, beta, A, x, y);                                               \
      Kokkos::Profiling::popRegion();                                                                                   \
    }                                                                                                                   \
  };
...
inline void spmv_mv_onemkl(const execution_space& exec, Handle* handle, oneapi::mkl::transpose mkl_mode,
  ...
  if (handle->tpl_rank1) {
    ...
    oneapi::mkl::sparse::optimize_gemm(exec.sycl_queue(), mkl_mode, subhandle->mat, {ev});
  }
  ...
  oneapi::mkl::sparse::gemm(exec.sycl_queue(), mkl_mode, oneapi::mkl::transpose::nontrans, alpha, subhandle->mat,
                            reinterpret_cast<const onemkl_scalar_type*>(x.data()),
			    static_cast<std::int64_t>(y.extent(1)), static_cast<std::int64_t>(x.stride(1)), beta,
                            reinterpret_cast<onemkl_scalar_type*>(y.data()), static_cast<std::int64_t>(y.stride(1)));
}

#define KOKKOSSPARSE_SPMV_MV_ONEMKL(SCALAR, ORDINAL, MEMSPACE)                                                         \
  template <>                                                                                                          \
  struct SPMV_MV<                                                                                                      \
      Kokkos::SYCL,                                                                                                    \
    ...
    using coefficient_type = typename YVector::non_const_value_type;                                                   \
                                                                                                                       \
    static void spmv_mv(const execution_space& exec, Handle* handle, const char mode[], const coefficient_type& alpha, \
                        const AMatrix& A, const XVector& x, const coefficient_type& beta, const YVector& y) {          \
      std::string label = "KokkosSparse::spmv_mv[TPL_ONEMKL," + KokkosKernels::ArithTraits<SCALAR>::name() + "]";      \
      Kokkos::Profiling::pushRegion(label);                                                                            \
      oneapi::mkl::transpose mkl_mode = mode_kk_to_onemkl(mode[0]);                                                    \
      spmv_mv_onemkl(exec, handle, mkl_mode, alpha, A, x, beta, y);                                                    \
      Kokkos::Profiling::popRegion();                                                                                  \
    }                                                                                                                  \
  };

KOKKOSSPARSE_SPMV_MV_ONEMKL(float, std::int32_t, Kokkos::SYCLDeviceUSMSpace)
KOKKOSSPARSE_SPMV_MV_ONEMKL(double, std::int32_t, Kokkos::SYCLDeviceUSMSpace)
/*
KOKKOSSPARSE_SPMV_MV_ONEMKL(Kokkos::complex<float>, std::int32_t, Kokkos::SYCLDeviceUSMSpace)
KOKKOSSPARSE_SPMV_MV_ONEMKL(Kokkos::complex<double>, std::int32_t, Kokkos::SYCLDeviceUSMSpace)
*/

KOKKOSSPARSE_SPMV_MV_ONEMKL(float, std::int64_t, Kokkos::SYCLDeviceUSMSpace)
KOKKOSSPARSE_SPMV_MV_ONEMKL(double, std::int64_t, Kokkos::SYCLDeviceUSMSpace)
/*
KOKKOSSPARSE_SPMV_MV_ONEMKL(Kokkos::complex<float>, std::int64_t, Kokkos::SYCLDeviceUSMSpace)
KOKKOSSPARSE_SPMV_MV_ONEMKL(Kokkos::complex<double>, std::int64_t, Kokkos::SYCLDeviceUSMSpace)
*/
#endif
